...
class RRT(object):
    def __init__(self):
        ...
        self.odom_sub = rospy.Subscriber("/odom", Odometry, self.pf_callback, buff_size=1)
    ...
    def pf_callback(self, pose_msg):
        ...
        goal_magnitudes = np.asarray([LA.norm(np.asarray([goal_point[0], goal_point[1]]) - car_point) for goal_point in goal_points])
        ...
        x = (tree_x - car_point[0])*math.cos(angle_z) + (tree_y - car_point[1])*math.sin(angle_z)
        y = -(tree_x - car_point[0])*math.sin(angle_z) + (tree_y - car_point[1])*math.cos(angle_z)
