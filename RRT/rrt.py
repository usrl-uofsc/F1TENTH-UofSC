...
Waypoint_CSV_File_Path = '/home/anandambrosi/f1tenth_labs-master/lab7/code/waypoints/centerline_300main.csv'
...
class RRT(object):
    def __init__(self):
        ...
        f = open('/home/anandambrosi/f1tenth_labs-master/lab7/code/maps/300main_clean2.pgm', 'rb')
    ...
    def pf_callback(self, pose_msg):
        ...
        for i in unaligned_waypoints:
            ...
            waypoints.append(car_x)
            waypoints.append(car_y)
            waypoints.append(angle_z)
    ...
    def steer_modified(self, nearest_node, sample_point, tree, x_index, y_index):
        ...
        new_x_index = int(round(np.interp(self.node_length, [0,dist], [node_x, sample_x])))
        new_y_index = int(round(np.interp(self.node_length, [0,dist], [node_y, sample_y])))
    ...
    # def check_collision_modified(self, new_node, nearest_node, tree):

        node_length = 10
        k = car_width*2 + node_length
        maj_a = k/2

        xf1 = new_node[0]
        yf1 = new_node[1]

        if self.occ_grid[xf1][yf1][0] == 1:
            return True

        xf2 = tree[nearest_node].x
        yf2 = tree[nearest_node].y

        xc = (xf1+xf2)/2
        yc = (yf1+yf2)/2

        dc = math.sqrt((xf1-xf2)**2 + (yf1-yf2)**2)/2
        min_a = math.sqrt(abs(maj_a**2 - dc **2))

        if xf1 == xf2:
            phi = math.pi/2
        else:
            phi = math.atan(abs(yf1 - yf2) / abs(xf1-xf2))

        for i in self.rad_matrix:
            xtt = xc + maj_a*math.cos(i)*math.cos(phi) - min_a*math.sin(i)*math.sin(phi)
            xtb = xc + maj_a*math.cos(-i)*math.cos(phi) - min_a*math.sin(-i)*math.sin(phi)
            yt = yc + maj_a*math.cos(i)*math.sin(phi) + min_a*math.sin(i)*math.cos(phi)
            xtb = int(round(xtb))
            xtt = int(round(xtt))
            yt = int(round(yt))
            for i in range(xtb, xtt):
                xt = i
                if self.occ_grid[xt][yt][0] == 1:
                    return True
